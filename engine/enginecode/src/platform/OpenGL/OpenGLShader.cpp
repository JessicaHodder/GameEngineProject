...
namespace Engine
{
	OpenGLShader::OpenGLShader(const char* vertexFilepath, const char* fragmentFilepath)
	{
		...
		{
			Log::error("Could not open shader vertex soruce: {0}", vertexFilepath);
			return;
		}
		handle.close();
		...
		{
			Log::error("Could not open shader fragment soruce: {0}", fragmentFilepath);
			return;
		}
		handle.close();
	...
	OpenGLShader::OpenGLShader(const char* filepath)
	{
		...
		{
			Log::error("Could not open shader vertex soruce: {0}", filepath);
			return;
		}
		handle.close();
